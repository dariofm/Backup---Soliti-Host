object frmPrincipal: TfrmPrincipal
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Kimmera Backup'
  ClientHeight = 447
  ClientWidth = 572
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  PopupMenu = pm1
  Position = poDesktopCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 8
    Top = 5
    Width = 41
    Height = 13
    Caption = 'Empresa'
  end
  object Label2: TLabel
    Left = 271
    Top = 5
    Width = 22
    Height = 13
    Caption = 'Cnpj'
  end
  object Label3: TLabel
    Left = 8
    Top = 190
    Width = 34
    Height = 13
    Caption = 'Origem'
  end
  object Label4: TLabel
    Left = 8
    Top = 303
    Width = 63
    Height = 13
    Caption = 'Destino Local'
  end
  object hora: TLabel
    Left = 444
    Top = 327
    Width = 3
    Height = 13
    Visible = False
  end
  object btnArquivo: TButton
    Left = 8
    Top = 164
    Width = 25
    Height = 25
    ImageIndex = 1
    Images = cxImageList1
    TabOrder = 2
    TabStop = False
    OnClick = btnArquivoClick
  end
  object btn2: TButton
    Left = 36
    Top = 164
    Width = 25
    Height = 25
    ImageIndex = 0
    Images = cxImageList1
    TabOrder = 3
    TabStop = False
    OnClick = btn2Click
  end
  object dbgrd1: TDBGrid
    Left = 8
    Top = 208
    Width = 556
    Height = 89
    DataSource = dsArquivo
    Options = [dgTitles, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    TabOrder = 4
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'DESCRICAO'
        Title.Caption = 'Arquivos de Origem'
        Width = 450
        Visible = True
      end>
  end
  object btnRemover: TButton
    Left = 66
    Top = 164
    Width = 25
    Height = 25
    ImageIndex = 2
    Images = cxImageList1
    TabOrder = 5
    TabStop = False
    OnClick = btnRemoverClick
  end
  object dbgrd2: TDBGrid
    Left = 8
    Top = 80
    Width = 556
    Height = 81
    DataSource = dsTarefa
    Options = [dgTitles, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    TabOrder = 6
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'TIME'
        Title.Caption = 'Hor'#225'rio do Backup'
        Width = 141
        Visible = True
      end>
  end
  object edtTime: TcxTimeEdit
    Left = 8
    Top = 53
    TabOrder = 7
    Width = 74
  end
  object btnAddTarefa: TButton
    Left = 88
    Top = 51
    Width = 25
    Height = 25
    ImageIndex = 3
    Images = cxImageList1
    TabOrder = 8
    OnClick = btnAddTarefaClick
  end
  object btn1: TButton
    Left = 117
    Top = 51
    Width = 28
    Height = 25
    ImageIndex = 2
    Images = cxImageList1
    TabOrder = 9
    OnClick = btn1Click
  end
  object edtEmpresa: TcxDBTextEdit
    Left = 8
    Top = 24
    DataBinding.DataField = 'DESCRICAO'
    DataBinding.DataSource = dsEmpresa
    Properties.CharCase = ecUpperCase
    TabOrder = 0
    OnExit = edtEmpresaExit
    Width = 257
  end
  object btn3: TButton
    Left = 399
    Top = 22
    Width = 25
    Height = 25
    ImageIndex = 4
    Images = cxImageList1
    TabOrder = 10
    TabStop = False
    Visible = False
    OnClick = btn3Click
  end
  object edtCnpj: TcxDBSpinEdit
    Left = 271
    Top = 24
    DataBinding.DataField = 'CNPJ'
    DataBinding.DataSource = dsEmpresa
    Properties.SpinButtons.Visible = False
    TabOrder = 1
    OnExit = edtCnpjExit
    Width = 121
  end
  object btnRemoverLocal: TButton
    Left = 38
    Top = 322
    Width = 25
    Height = 25
    ImageIndex = 2
    Images = cxImageList1
    TabOrder = 11
    TabStop = False
    OnClick = btnRemoverLocalClick
  end
  object btnDiretorioDestino: TButton
    Left = 8
    Top = 322
    Width = 25
    Height = 25
    ImageIndex = 0
    Images = cxImageList1
    TabOrder = 12
    TabStop = False
    OnClick = btnDiretorioDestinoClick
  end
  object dbgrd3: TDBGrid
    Left = 8
    Top = 352
    Width = 556
    Height = 89
    DataSource = dsDestino
    Options = [dgTitles, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    TabOrder = 13
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'DIRETORIO'
        Title.Caption = 'Destino Local'
        Width = 450
        Visible = True
      end>
  end
  object btn4: TButton
    Left = 475
    Top = 4
    Width = 89
    Height = 25
    Caption = 'Configura'#231#227'o'
    ImageIndex = 5
    Images = cxImageList1
    TabOrder = 14
    OnClick = btn4Click
  end
  object btn5: TButton
    Left = 475
    Top = 35
    Width = 89
    Height = 25
    Caption = 'Enviar'
    ImageIndex = 6
    Images = cxImageList1
    TabOrder = 15
    OnClick = btn5Click
  end
  object dlgOpen1: TOpenDialog
    Left = 160
    Top = 96
  end
  object SQLiteUniProvider1: TSQLiteUniProvider
    Left = 296
    Top = 160
  end
  object UniConnection1: TUniConnection
    ProviderName = 'SQLite'
    Database = 'C:\Kimmera Backup\cloud.db'
    Connected = True
    Left = 376
    Top = 160
  end
  object qArquivos: TUniQuery
    Connection = UniConnection1
    SQL.Strings = (
      'select * from ARQUIVOS')
    Left = 248
    Top = 240
    object qArquivosID: TIntegerField
      AutoGenerateValue = arAutoInc
      FieldName = 'ID'
    end
    object qArquivosDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 200
    end
    object qArquivosTIPO: TStringField
      FieldName = 'TIPO'
      Size = 10
    end
    object qArquivosDIRETORIO: TStringField
      FieldName = 'DIRETORIO'
      Size = 30
    end
  end
  object dsArquivo: TUniDataSource
    DataSet = qArquivos
    Left = 320
    Top = 240
  end
  object qTarefa: TUniQuery
    Connection = UniConnection1
    SQL.Strings = (
      'select * from TAREFA')
    Left = 224
    Top = 96
    object qTarefaID: TIntegerField
      AutoGenerateValue = arAutoInc
      DisplayWidth = 10
      FieldName = 'ID'
    end
    object qTarefaTIME: TTimeField
      DisplayWidth = 10
      FieldName = 'TIME'
    end
  end
  object dsTarefa: TUniDataSource
    DataSet = qTarefa
    Left = 288
    Top = 96
  end
  object cxImageList1: TcxImageList
    SourceDPI = 96
    FormatVersion = 1
    DesignInfo = 3670456
    ImageInfo = <
      item
        ImageClass = 'TdxPNGImage'
        Image.Data = {
          
          A14B22EE620A0000000049454E44AE426082}
      end
      item
        ImageClass = 'TdxPNGImage'
        Image.Data = {
          
          BB81AB7F56B30000000049454E44AE426082}
      end
      item
        ImageClass = 'TdxPNGImage'
        Image.Data = {
          
          420857CF290000000049454E44AE426082}
      end
      item
        ImageClass = 'TdxPNGImage'
        Image.Data = {
          }
      end
      item
        ImageClass = 'TdxPNGImage'
        Image.Data = {
          
          F6AC9379188DF20000000049454E44AE426082}
      end
      item
        ImageClass = 'TdxPNGImage'
        Image.Data = {
          
          82}
      end
      item
        ImageClass = 'TdxPNGImage'
        Image.Data = {
          
          69B57B8C41DD0000000049454E44AE426082}
      end>
  end
  object tEmpresa: TUniTable
    TableName = 'EMPRESA'
    Connection = UniConnection1
    Left = 336
    Top = 16
    object tEmpresaDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 60
    end
    object tEmpresaCNPJ: TStringField
      FieldName = 'CNPJ'
    end
  end
  object dsEmpresa: TUniDataSource
    DataSet = tEmpresa
    Left = 288
    Top = 16
  end
  object tDestino: TUniTable
    TableName = 'DESTINO'
    Connection = UniConnection1
    Active = True
    Left = 216
    Top = 384
    object tDestinoID: TIntegerField
      AutoGenerateValue = arAutoInc
      FieldName = 'ID'
    end
    object tDestinoDIRETORIO: TStringField
      FieldName = 'DIRETORIO'
      Size = 200
    end
  end
  object dsDestino: TUniDataSource
    DataSet = tDestino
    Left = 280
    Top = 384
  end
  object tmrDisparador: TTimer
    OnTimer = tmrDisparadorTimer
    Left = 264
    Top = 304
  end
  object tMega: TUniTable
    TableName = 'MEGA'
    Connection = UniConnection1
    Active = True
    Left = 416
    Top = 240
    object tMegaEMAIL: TStringField
      FieldName = 'EMAIL'
      Size = 50
    end
    object tMegaSENHA: TStringField
      FieldName = 'SENHA'
      Size = 50
    end
  end
  object TrayIcon1: TTrayIcon
    OnClick = TrayIcon1Click
    Left = 456
    Top = 312
  end
  object ApplicationEvents1: TApplicationEvents
    OnMinimize = ApplicationEvents1Minimize
    Left = 384
    Top = 312
  end
  object tmrAutoMinimiza: TTimer
    OnTimer = tmrAutoMinimizaTimer
    Left = 160
    Top = 304
  end
  object pm1: TPopupMenu
    Left = 504
    Top = 152
    object Host1: TMenuItem
      Caption = 'Host'
      OnClick = Host1Click
    end
    object Soliti1: TMenuItem
      Caption = 'Soliti'
      OnClick = Soliti1Click
    end
  end
  object qGen: TUniQuery
    Connection = UniConnection1
    Left = 104
    Top = 256
  end
  object uConFire: TUniConnection
    ProviderName = 'InterBase'
    Username = 'SYSDBA'
    Server = '127.0.0.1'
    Left = 504
    Top = 240
    EncryptedPassword = '92FF9EFF8CFF8BFF9AFF8DFF94FF9AFF86FF'
  end
  object InterBaseUniProvider1: TInterBaseUniProvider
    Left = 312
    Top = 272
  end
  object qEmit: TUniQuery
    Connection = uConFire
    Left = 504
    Top = 304
  end
end
